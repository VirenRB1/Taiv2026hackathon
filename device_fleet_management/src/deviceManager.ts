...
export class DeviceManager {
  ...
  removeDevice(id: string): void {
    //device id isnt in the lsit throw error
  ...
  private calculateDistance(lat1: number, lon1: number, lat2: number, lon2: number): number {
    // Had to use alittle bit of AI help with the math here
  ...
  getDevicesNearDevice(device_id: string, radius_km: number): Device[] | null {
    ...
    const results: Device[] = [];
    for (const device of this.devices.values()) {
      if (device.id === device_id) continue; // Exclude itselfr
      ...
      const dist = this.calculateDistance(
        ...
        //if the device isin radius add it to the liost
        if (dist <= radius_km) {
        results.push(device);
      }
    }
